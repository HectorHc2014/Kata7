...
public class Watch {
    ...
    public Watch(double seconds, double minutes, double hours) {
        Timer timer = new Timer();
        timer.schedule(timerTask(), 0, 1000);
        this.hours=(hours%12)*(-30*Math.PI/180) + Math.PI / 2 -(30*Math.PI/180)*(minutes/60) ;
        this.minutes=minutes*(-6*Math.PI/180) + Math.PI / 2;
        this.seconds=seconds*(-6*Math.PI/180) + Math.PI / 2;
    }
